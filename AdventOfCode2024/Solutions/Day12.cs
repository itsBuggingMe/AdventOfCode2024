        ...
        new HashSet<int>() is { } large ?
            ...
            self is not Func<object, string, int, int, char, int[], HashSet<int>, HashSet<int>, HashSet<int>> func ?
                set : deltas
                    ...
                    (set: func(func, input, inputWidth, c.i, c.c, deltas, new(), large), c.c) : throw null!)
            .Where(t => t.set.Count != 0)
            .Sum(t => t.set.SelectMany(p => deltas.Where(d => p + d is int x && (x < 0 || x >= input.Length || input[x] != t.c))
            .Select(d => (normal: d, pos: p)))
            .GroupBy(a => Math.Abs(a.normal) == 1 ? (a.normal, a.pos % (inputWidth + 1)) : (a.normal, a.pos / (inputWidth + 1)))
                .Select(a => a.OrderBy(c => c.pos)).Sum(a => a.Skip(1)
                    .Aggregate(
                        (sum: 1, prev: a.First().pos, expected: Math.Abs(a.First().normal) == 1 ? (inputWidth + 1) : 1),
                        (a, c) => (sum: a.sum + Convert.ToInt32(a.prev + a.expected != c.pos), prev: c.pos, a.expected)).sum) * t.set.Count)
        : throw null!;
}